((helpLib) => {
	...
	helpLib.regHelper('num', 'is', null, (num) => {
		let type = typeof num;
		switch(type) {
	...
	helpLib.regHelper('num', 'isInt', null, (num) => {
		if(this.num.is(num)) {
	...
	helpLib.regHelper('num', 'check', null, (num, defValue = 0) => {
		return this.num.is(num) ? Number(num) : defValue;
	});

	helpLib.regHelper('num', 'random', null, (min = 0, max = Number.MAX_SAFE_INTEGER) => {
		min = this.num.check(min);
		max = this.num.check(max);
		return Math.round(min - 0.5 + Math.random() * (max - min + 1));
	});

	helpLib.regHelper('num', 'format', {str: 'check'}, (value, intSize, fractSize) => {
	...
	helpLib.regHelper('num', 'inInterval', null, (num, minValue, maxValue, defValue = null) => {
	...
	helpLib.regHelper('num', 'toFlag', null, (num) => {
